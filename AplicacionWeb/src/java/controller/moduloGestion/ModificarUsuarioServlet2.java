...
public class ModificarUsuarioServlet2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
          ...
          ResultSet rs = conexion.consultarRegistro("SELECT * FROM USUARIOS WHERE ID="+id);
          if (!rs.getString(2).contains(nombre))
             conexion.ejecutarQuery("UPDATE USUARIOS SET NOMBRE='"+nombre+"' WHERE ID="+id);          
          if (!rs.getString(3).contentEquals(apellido))
              conexion.ejecutarQuery("UPDATE USUARIOS SET APELLIDO='"+apellido+"' WHERE ID="+id);
          if (!rs.getString(4).contentEquals(correo))
              conexion.ejecutarQuery("UPDATE USUARIOS SET CORREO='"+correo+"' WHERE ID="+id);
          ...
          request.setAttribute("mensaje","Usuario Modificado");
           RequestDispatcher dispatcher = request.getRequestDispatcher("/respuesta.jsp");
            ...
            Logger.getLogger(ModificarUsuarioServlet2.class.getName()).log(Level.SEVERE, null, ex);
        } finally {            
            out.close();
        }
    }
